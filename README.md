...
- 因为当前 macOS 构建未使用 Developer ID 签名，应用内自动安装不可用；请使用 `brew upgrade --cask prokcy` 或手动下载 DMG 升级
...
当前 macOS 安装包是**未签名**构建。若提示「无法验证开发者」，请在 Finder 中右键应用并选择「打开」；若提示「已损坏」，通常是拿到了旧的 ad-hoc 签名产物，请前往 [Releases](https://github.com/Yaphet2015/Prokcy/releases) 重新下载最新版本。升级推荐使用 `brew upgrade --cask prokcy`。
</details>
