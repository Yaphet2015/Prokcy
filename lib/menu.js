...
const {
  Menu, app, Tray, shell, nativeImage, nativeTheme,
} = require('electron');
...
const hideDockMenu = {
  ...
  icon: icons.uncheck,
  click() {
    updateHideFromDock(!getHideFromDock()); // eslint-disable-line
  },
};

const updateHideFromDock = (hide) => {
  if (hide === undefined) {
    hide = getHideFromDock();
  }
  hide = setHideFromDock(hide);
  if (updateCheckbox) {
...
exports.create = async () => {
  ...
  updateTrayMenus = () => {
    ...
    tray.setContextMenu(Menu.buildFromTemplate(trayRulesMenu.concat(trayMenus))); // eslint-disable-line
  };
  updateCheckbox = (menu, checked) => {
    ...
    } else if (menu === startMenu && startAppMenuItem) {
    ...
    if (createAppMenus) {
      Menu.setApplicationMenu(Menu.buildFromTemplate(createAppMenus()));
    }
    updateTrayMenus();
  };
  ...
  const checkUpdate = async () => {
    ...
    try {
      ...
      if (version === newVersion) {
        return showMessageBox('Prokcy is up to date', {
          title: '',
          type: 'info',
        });
      }
      return showMessageBox(`Prokcy has new version ${newVersion}`, {
        type: 'info',
        title: '',
  ...
  const proxyAppMenuItem = {
    label: 'Set As System Proxy',
    click: switchSystemProxy,
  };
  const startAppMenuItem = {
    label: 'Start At Login',
    click: switchStartAtLogin,
  };
  const hideDockAppMenuItem = {
    label: 'Hide From Dock',
    click() {
...
exports.refreshProxyStatus = () => {
  if (updateCheckbox) {
    updateCheckbox(proxyMenu, isEnabled());
    if (updateTrayMenus) {
      updateTrayMenus();
    }
  }
