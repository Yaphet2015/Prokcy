...
const getRulesPayload = (proxy) => {
  ...
  if (typeof proxy.rulesUtil.rules.getAllData === 'function') {
    ...
    if (all && Array.isArray(all.list)) {
      base.list = all.list;
      if (typeof all.backRulesFirst === 'boolean') {
        base.backRulesFirst = all.backRulesFirst;
      }
      if (typeof all.allowMultipleChoice === 'boolean') {
        base.allowMultipleChoice = all.allowMultipleChoice;
      }
    }
  }
  return base;
...
process.parentPort.on('message', (data) => {
  ...
  if (type === 'setRulesContent') {
    ...
    if (!name || /^default$/i.test(name)) {
      return proxy.rulesUtil.rules.setDefault(data.content || '');
    }
    return proxy.rulesUtil.rules.add(name, data.content || '');
  }
  if (type === 'disableAllPlugins') {
  ...
  if (type === 'setAllowMultipleChoice') {
    proxy.rulesUtil.properties.set('allowMultipleChoice', !!data.enabled);
    proxy.emit('rulesDataChange', 'allowMultipleChoice', !!data.enabled);
    return;
  }
  if (type === 'setBackRulesFirst') {
    proxy.rulesUtil.rules.enableBackRulesFirst(!!data.enabled);
    return;
  }
