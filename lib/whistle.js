...
process.parentPort.on('message', (data) => {
  ...
  if (type === 'setRulesContent') {
    ...
    if (!name || /^default$/i.test(name)) {
      proxy.rulesUtil.rules.setDefault(data.content || '');
    } else {
      proxy.rulesUtil.rules.add(name, data.content || '');
    }
    // Explicitly trigger rules reload after add()
    // This is needed because rules.add() doesn't call parseRules() internally,
    // unlike setDefault() which does. Calling parseRules() again is harmless
    // for Default rules (idempotent) but essential for custom rule groups.
  ...
  if (type === 'renameRulesGroup') {
    ...
    if (!newName || /^default$/i.test(newName)) {
      return { success: false, message: 'Invalid new name' };
    }
    if (!proxy.rulesUtil.rules.exists(name)) {
      return { success: false, message: 'Group does not exist' };
    }
